...
namespace SnowAPI.Controllers
{
    ...
    public class RouteController : BaseController
    {
        ...
        [HttpGet]
        [Route("points")]
        public async Task<IActionResult> GetPoints([FromQuery] int? pagesize, [FromQuery] int? page, [FromQuery] string routes)
        {
            var model = new PagingRequestModel(page, pagesize);

            var result = await _routeService.GetPoints(model, routes);
